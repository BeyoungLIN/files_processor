...
def pic_compose(page_list, root):
    ...
    for pn, p in enumerate(page_list[:2]):
        ...
        cv2.imwrite(f"{pdf_folder}/{name}.png", rimg_tmp)
...
def get_pdf_pic_ls(pdf_folder):
    ...
    # pdf_pic_ls = filter(lambda x: x.endswith('.png'), pdf_pic_ls_)   #将jpg文件读进来，可改为jng等格式
...
def creat_pdf(pdf_folder, pdf1_filename):
    ...
    for each_img_pth in pdf_pic_pth_ls:
        # 得到完整的图片路径
    ...
    del list1[0]
    print(len(list1))
    im1.save(pdf1_filename, "PDF", resolution=100.0, save_all=True, append_images=list1)
...
if __name__ == '__main__':
    ...
    page_list = divi_pic(pic_ls)
    pdf_folder = pic_compose(page_list, root)
    # pdf_pic_ls = get_pdf_pic_ls(pdf_folder)
    pdf1_filename = '/Users/Beyoung/Desktop/Projects/u3dprg课件_4.pdf'
